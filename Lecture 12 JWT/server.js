...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    const verfiedToken = generateToken(email);

    sendEmailVerificationMail({ email, verfiedToken });
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    //check for email verfied
    if (!userDb.isEmailVeified)
...
app.get("/verify/:token", async (req, res) => {
  ...
  try {
    await userModel.findOneAndUpdate({ email }, { isEmailVeified: true });
